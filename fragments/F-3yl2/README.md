...
These are parts of physical rows 2–3 of that crop. Proposed expansions appear in parentheses. The [Corpus Thomisticum text, q.10 a.12 corpus, paragraph39269](https://www.corpusthomisticum.org/sth3001.html) reasons from civil ownership to the natural relation between parent and child, then distinguishes a child's condition after attaining use of free choice. It says *contra iustitiam naturalem esset si puer … a cura parentum subtrahatur*, not the fragment's compact *filium carentem usu rationis auferre parentibus*. This is a close argument/wording connection with condensation, not a verbatim copy.

A separate manuscript crop centred on the legal references reads across its first two rows:

> … ut l(ege) finali C(odice) de i(m)puberib(us) et aliis su[b]- / stitutio(n)ib(us) …

The following causa/quaestio numeral is uncertain and is not expanded into an exact canon here. The explicit Codex reference does not occur in the compared Aquinas paragraph. It supports a modified or compiled text; it does not identify a particular compiler or establish legal training biographically.

## Additional check: heresy

The bottom front-cover crop shows, in its final two text rows, the selected sequence:
