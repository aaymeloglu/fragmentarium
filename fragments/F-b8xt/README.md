...
Recto b, foot of column, in the corollary comparing substance and quality:

```text
r.b41 ubi q(ue)r(itur): q(uod)[?] si q(ue)ra(tur) quid e(st) h(om)o (et) r(espon)d(etur)
r.b42 a(n)i(m)al, li a(n)i(m)al q(uia) e(st) in g(e)n(er)e s(ubstanti)e no(n) sol(um)
r.b43 d(icit) q(ui)d e(st) h(om)o s(ed) (et)ia(m)[?] a(bsolu)te s(ignifica)t q(ui)d, i(d) e(st)
r.b44 s(ubstanti)am. S(ed) cu(m) q(ue)r(itur) q(ui)d e(st) alb(e)do (et) r(espon)d(etur)
r.b45 q(uod) e(st) color, li color li(cet) s(ignifice)t quid e(st)
r.b46 alb(e)do, t(ame)n no(n) a(bsolu)te s(ignifica)t quid s(ed) q(ua)le,
r.b47 (et) i(de)o q(ua)litas no(n) h(abe)t q(ui)d si(mpliciter) s(ed) s(ecundu)m q(ui)d
r.b48 ut color e(st) q(ui)d alicuius[?], (et) h(oc) q(ui)d magis
r.b49 p(otes)t[?] dici s(ubstanti)ale q(uam) s(ubstanti)a; q(uia) cu(m)[?]
...
```text
r.b51 [i(de)o?] [...] mod(us) e(nt)it(a)t(is) s(ubstanti)e .s. e(ss)e quid
r.b52 p(ar)ticipat(ur) s(ecundu)m q(ua)nda(m) si(mi)litudi(n)e(m) p(ro)p(or)tio(n)is
r.b53 in o(mn)ib(us) aliis p(re)dic(ament)is, ut sic(ut) a(n)i(m)al e(st)
v.a01 quid e(st) h(om)o, ita color e(st) quid e(st) al-
v.a02 bedinis (et) nu(mer)us e(st) quid e(st) dualit(a)t(is), (et)
```

The cramped opening of r.b51 is left incomplete. The edition has a fully intelligible sentence there, but that does not license filling the manuscript gap. The repeated *quid est* in the manuscript continuation is retained instead of silently adopting the edition's shorter genitive constructions.

A separate passage at the top of verso b helps define the unresolved compiling layer:

```text
v.b01 3a (con)cl(usi)o. diff(initio) (et) q[...] q[...] erat e(ss)e q(uid)ditative[?] su(n)t
v.b02 (et) p(ro)p(ri)e solis sp(ecie)b(us) sp(ecialissi)mis de g(e)n(er)e
v.b03 s(ubstanti)e. p(ate)t p(ri)m(o) p(er) p(hilosophu)m in l(itte)ra. [...] sp(ecie)s
v.b04 dicu(n)t aliq(ui)d[?] [...] naturam seu esse(n)tia(m) que
...
The checked source is Aquinas, *Sententia libri Metaphysicae* VII, lectio 4, nn. 3–4, Corpus Thomisticum paragraphs [82899]–[82900]. The [digital edition](https://www.corpusthomisticum.org/cmp07.html) states that it presents the Turin 1950 text, subsequently processed and revised by Roberto Busa and Enrique Alarcón. The cached HTML and live text were read; neighbouring paragraphs 1–10 were also inspected to assess the context.

| Manuscript passage | Result of comparison |
|---|---|
| r.b41–47: man/animal, then whiteness/colour, then *quid* versus *quale* | The examples, explanatory clauses, and order closely follow n. 3. The manuscript begins *si queratur* where Aquinas has *cum enim quaero*, adds *li* before *color*, and has *tamen non* in place of *non tamen*. |
| r.b48–49: *magis ... substantiale quam substantia* | The unusual comparison corresponds to n. 3, but the apparent manuscript *potest dici* differs from Aquinas's *est*. The uncertain *alicuius* should not be normalized to Aquinas's *albedinis*. |
| r.b50–53: dependent being and proportional similarity | The categories' dependence on substance is followed in both texts by participation in its mode of being through a similarity of proportion. This extended explanatory sequence strengthens the source connection beyond a shared Aristotelian example. |
| v.a1–2: man, whiteness, duality | The sequence follows n. 4, while the manuscript repeats *quid est* where Aquinas uses shorter constructions such as *quid hominis*. |
| v.b1–5: third conclusion and lowest species | The manuscript has numbered conclusion apparatus not present at the checked Aquinas locus. No exact textual match was established for this additional passage. |
